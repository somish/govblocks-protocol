...
contract('GBTStandardToken', function([owner, taker]) {
  ...
  it('should be initialized', async function() {
    ...
    assert.equal(
      await gbts.tokenPrice(),
      1000000000000000,
      'tokenPrice was not set properly'
    );
  });
  it('should buy/mint 100 GBT', async function() {
    ...
    assert.equal(
      await gbts.balanceOf(owner),
      100000000000000000000,
      'GBT was not mint properly'
    );
    assert.equal(
      await gbts.totalSupply(),
      100000000000000000000,
      'Total Supply not set properly'
    );
  });
  ...
  it('should increase and decrease allowance', async function() {
    ...
    assert.equal(
      allowance2.toNumber(),
      allowance.toNumber() + amount,
      'allowance not increased successfully'
    );
    ...
    assert.equal(
      allowance2.toNumber(),
      allowance.toNumber(),
      'allowance not decreased successfully'
    );
  });
  ...
  it('should close minting', async function() {
    this.timeout(100000);
    assert.equal(
      await gbts.mintingFinished(),
      false,
      'minting finished already'
    );
  ...
  it('should transfer from with message', async function() {
    ...
    var result = await gbts.transferFromMessage(
      owner,
      memberAddress,
      amount,
      '0x46',
    ...
    assert.equal(
      b22.toNumber(),
      b21.toNumber(),
      'Amount taken/given to the wrong guy'
    );
    assert.equal(
      await gbts.allowance(owner, taker),
      allowance - amount,
      "Allowance wasn't decreased"
    );
  });
});
