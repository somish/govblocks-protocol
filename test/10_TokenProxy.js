...
contract('TokenProxy', function([owner, receiver, spender]) {
  ...
  it('should proxy correct data', async function() {
    this.timeout(100000);
    tp = await TokenProxy.new(gbts.address,1e18);
  ...
  it('should allow transfer with lock again after claiming', async () => {
    ...
    assert.equal((await tp.tokensLocked(receiver, lockReason3)).toNumber(), 0);
  ...
  it('should not allow 0 address', async () => {
    await catchRevert(TokenProxy.new(nullAddress,1e18));
